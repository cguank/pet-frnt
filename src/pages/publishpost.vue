<template>
  <div>
    ...
    <van-cell-group v-if="this.$route.query.need_title">
...
<script>
  import Vue from 'vue';
  import {Row, Col, Divider, Icon, Uploader, Toast, Field, CellGroup, Button} from 'vant';

  Vue.use(Row).use(Col).use(Divider).use(Icon).use(Uploader).use(Field).use(CellGroup).use(Button);
  ...
  export default {
    ...
    methods: {
      ...
      publish() {
        ...
        this.$postData('insertPost',{
          'title': this.title,
          'comment': msg,
          'imgListStr': this.fileListStr
        }).then(res => {  // 返回该帖子id，用于插入附件
          if(res.data){
            Toast('发布成功');
